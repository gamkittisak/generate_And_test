class DataGen extends GWinData {
  ...
  void cleanColor() {
    int text = #ffffff;
    hue.setPositionSlider(hue(text));
    hue.setColor(hue(text));
    saturation.setPositionSlider(saturation(text));
    saturation.setColor(saturation(text));
    brightness.setColor(brightness(text));
    brightness.setPositionSlider(brightness(text));
  }
  }